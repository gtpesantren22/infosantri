                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            while ($ar = mysqli_fetch_array($dtkls)) {
                                                ...
                                                $k_madin = $kl[0];
                                                $r_madin = $kl[1];
                                                $cek = mysqli_num_rows(mysqli_query($koneksi3, "SELECT * FROM tb_santri WHERE aktif = 'Y' AND k_madin = '$k_madin' AND r_madin = '$r_madin' AND jkl  = 'Perempuan' "));
                                                if ($cek > 0) {
                                                ?>
                                                    <a href="<?= 'cek_absenMd.php?kls=' . $klsx . '&tgl=' . $mgr['tanggal'] . '&jkl=Perempuan'  ?>" class="btn btn-primary btn-sm mb-1"><?= $klsx; ?></a>
                                            <?php }
                                            } ?>
                                        </div>
                                        <?php
                                            ...
                                            <div class="col-md-12">
                                                <div class="px-3 py-3 bg-gradient-light text-dark ">
                                                    ...
                                                    while ($ar = mysqli_fetch_array($dtkls)) {
                                                        ...
                                                        if ($cek > 0) {
                                                    ?>
                                                            <a href="<?= 'cek_absenMd.php?kls=' . $klsx . '&tgl=' . $tanggalKo . '&jkl=Perempuan'  ?>" class="btn btn-danger btn-sm mb-1"><?= $klsx; ?></a>
                                                    <?php }
                                                    } ?>
                                                </div>
                                            </div>
...
if (isset($_POST['buat'])) {
    ...
    if (mysqli_num_rows($cek2) > 0) {
        ...
        while ($in = mysqli_fetch_assoc($dt)) {
            $nis = $in['nis'];
            $input =  mysqli_query($conn, "INSERT INTO absen_md VALUES('', 'Perempuan', '$nis', '$bln', '$mg', '$range', '$tgl', '0', '0', '0', '-', '$ta')");
        }
        echo "
            <script>
            window.location = 'absen_madinPi.php';
