...
if ($level == 'madin') {
    $sql = mysqli_query($conn, "SELECT a.* FROM absen_md a JOIN tb_santri b ON a.nis=b.nis WHERE b.jkl= 'Laki-laki' AND b.aktif = 'Y' GROUP BY a.tanggal ORDER BY a.tanggal DESC LIMIT 1");
    $sql2 = mysqli_query($conn, "SELECT a.* FROM absen_md a JOIN tb_santri b ON a.nis=b.nis WHERE b.jkl= 'Laki-laki' AND b.aktif = 'Y' GROUP BY a.tanggal ORDER BY a.tanggal DESC ");
} else {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800"><b>DATA ABSENSI MADIN PUTRA</b></h1>
                    <hr>
...
if (isset($_POST['buat'])) {
    ...
    $cek = mysqli_query($conn, "SELECT * FROM absen_md WHERE bulan = $bln AND minggu = $mg AND ta = '$ta' AND jkl = 'Laki-laki' ");
    $cek2 = mysqli_query($conn, "SELECT * FROM absen_md WHERE tanggal = '$tgl' AND jkl = 'Laki-laki' ");
    if (mysqli_num_rows($cek2) > 0) {
        echo "
        <script>
        alert('Ada absen yang sudah dibuat pada tanggal ini. Silahkan buat ditanggal lain');
        window.location = 'absen_madinPa.php';
        </script>
        ";
        ...
        $dt = mysqli_query($conn, "SELECT * FROM tb_santri WHERE jkl = 'Laki-laki' AND aktif = 'Y' ");
        while ($in = mysqli_fetch_assoc($dt)) {
            $nis = $in['nis'];
            $input =  mysqli_query($conn, "INSERT INTO absen_md VALUES('', 'Laki-laki', '$nis', '$bln', '$mg', '$range', '$tgl', '0', '0', '0', '-', '$ta')");
        }
        echo "
            <script>
            window.location = 'absen_madinPa.php';
